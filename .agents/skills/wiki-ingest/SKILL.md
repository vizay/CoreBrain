...
Stage **only** the files touched during this ingest run — do not use `git add .`:

```bash
# Stage new/updated Wiki notes (one line per file created or updated in Step 4)
git add Wiki/<note-1>.md Wiki/<note-2>.md ...

# Stage the source file (processed: true written in Step 5)
git add Raw/Sources/<source-filename>.md

# Stage the two files wiki_tool.py writes to disk:
git add Wiki/catalog.jsonl   # regenerated by `build`
git add Wiki/log.md          # appended by `log`

git commit -m "ingest: <Source Title> → N wiki notes"
git push
```

> ⚠️ If `git push` fails (e.g. remote has diverged), report the error verbatim to the user.
> Do **not** force-push or rebase without explicit user instruction.
...
- Confirm the commit SHA and that the push succeeded (or report any push error)
