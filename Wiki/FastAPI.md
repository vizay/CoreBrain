...
- [[Core: Pydantic]] — for data validation and settings management
- [[Core: Starlette]] — for the ASGI web layer (routing, middleware, WebSockets)

## Key Points

- **Performance**: Benchmarked on par with NodeJS and Go; one of the fastest Python frameworks, only below Starlette and [[Core: Uvicorn]] themselves.
...
- **Standards-based**: Fully compatible with [[Core: OpenAPI]] (formerly Swagger) and JSON Schema. Generates interactive Swagger UI (`/docs`) and ReDoc (`/redoc`) automatically.
...
The `[standard]` extra includes [[Core: Uvicorn]] with high-performance extras, `fastapi-cli`, HTTPX (for testing), Jinja2, and `python-multipart`.
...
- HTTP Basic auth
- Deeply nested JSON model validation (via [[Core: Pydantic]])
- GraphQL integration (Strawberry)
- WebSockets (via [[Core: Starlette]])
